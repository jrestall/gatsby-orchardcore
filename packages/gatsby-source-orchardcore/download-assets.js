...
const bar = new ProgressBar(`Downloading OrchardCore Assets [:bar] :current/:total :elapsed secs :percent`, {
  total: 0,
  width: 30
});
let totalJobs = 0;
...
const downloadOrchardCoreAssets = async gatsbyFunctions => {
  const {
    ...
    getNodes,
    pluginConfig
  } = gatsbyFunctions; // Any OrchardCoreAsset nodes will be downloaded, cached and copied to public/static
  // regardless of if you use `localFile` to link an asset or not.

  const orchardAssetNodes = getNodes().filter(n => n.internal.owner === `gatsby-source-orchardcore` && n.internal.type === `OrchardCoreAsset`);
  const host = pluginConfig.get(`host`);
  await Promise.all(orchardAssetNodes.map(async node => {
    totalJobs += 1;
    bar.total = totalJobs;
    let fileNodeID;
    const remoteDataCacheKey = `orchardcore-asset-${node.name}-${node.lastModifiedUtc}`;
    const cacheRemoteData = await cache.get(remoteDataCacheKey);
    const url = `${host}${node.path}`; // Avoid downloading the asset again if it's been cached
